...
p1 <- ggplot() +
  # geom_sf(data = coast_proj, fill = "beige") +
  geom_point(data = haul, aes(x = X, y = Y, col = temperature_at_gear_c_der), size = 0.3) +
...
all_df$term[which(all_df$term == "phi")] <- "Observation error SD"
...
ggplot(all_df, aes(cutoff, mean_estimate)) +
  geom_ribbon(aes(ymin = mean_estimate - 2 * mean_se, ymax = mean_estimate + 2 * mean_se), alpha = 0.3) +
  geom_line() +
