...
run_cv <- function(cutoff, bin_width, species, bins_to_folds = bin_to_folds) {
  ...
  if (class(fit_cv) != "try-error") {
    ...
    list(pars = tidy_pars, loglik = ll)
  }
}

df <- expand.grid(
  cutoff = round(exp(seq(log(12), log(175), length.out = 2))),
  # bin_width = seq(10, 150, by = 90),
  bin_width = 50,
