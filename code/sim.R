...
ret1 <- furrr::future_map_dfr(cutoffs, fit_sim_data)
title1 <- paste0(
  "N = ", N,
  ", range = ", .range,
  ", obs. SD = ", .phi,
...
filename1 <- paste0("figures/", gsub(" ", "", title1), ".pdf")

# one that doesn't show the issue: ----------------------
true <- data.frame(
  name = c("phi", "sigma_O", "range"),
  value = c(1, 1, 0.3)
)
...
set.seed(1)
dat$fold_id <- sample(1:10, nrow(dat), replace = TRUE)
ret2 <- furrr::future_map_dfr(cutoffs, fit_sim_data)
title2 <- paste0(
  "N = ", N,
  ", range = ", .range,
  ", obs. SD = ", .phi,
...
filename2 <- paste0("figures/", gsub(" ", "", title2), ".pdf")

# plots -------------------------------------------------
pivot_longer(ret1, cols = -n) |>
  filter(n > 61) |> 
  ggplot(aes(n, value)) +
  geom_point() +
  geom_line() +
  ...
  ggtitle(title1, subtitle = "10-fold cross validation")

ggsave(filename1, width = 8, height = 5)

pivot_longer(ret2, cols = -n) |>
  filter(n > 61) |> 
  ggplot(aes(n, value)) +
  geom_point() +
  geom_line() +
  ...
  geom_hline(data = true, mapping = aes(yintercept = value), lty = 2) +
  ggsidekick::theme_sleek() +
  ggtitle(title2, subtitle = "10-fold cross validation")

ggsave(filename2, width = 8, height = 5)
