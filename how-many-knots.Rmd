...
Because of the large number of model runs needed to evaluate our sensitivity analyses, we restricted our analysis to only use survey data from 2018 (n = 702 observations).
...
As a first example of the impact of mesh resolution on predictive performance, we constructed spatial models of ocean temperature, using data from June 2018 (n = 175 tows).
As there are generally strong non-linear trends between depth and temperature, we used a statistical model of the form
...
All meshes in our analyses used a non-convex boundary and offset based on 5% of the data (e.g. Figure 1).
...
There is an inverse relationship between cutoff distance and mesh resolution; we varied cutoff distances between 20--160km in increments of 2.5km (corresponding to 53--197 vertices).
...
Penalized complexity priors were used on the Matérn covariance parameters [@simpson_penalising_2015; @osgood-zimmerman_statistical_2021].
Specifically, priors on the Matérn spatial range ($\kappa$) and marginal standard deviation ($\sigma_{\omega}$) are constructed as a function of the tail probabilities [@simpson2017; @fuglstad2018].
We assigned priors so that $Pr(\kappa < 20) = 0.05$ and $Pr(\sigma_{\omega} > 5) = 0.05$.

### Quantifying performance

Observations were randomly assigned to training and test sets (25 observations used for testing, and the same test and training data used for all meshes).
As sensitivity analysis, we also applied true Leave-One-Out Cross-Validation (LOOCV), with each observation alternatively held out as the test set.
...
We first applied the conditional predictive ordinate (CPO, [@pettit1990]), which is an approximation to the LOOCV.
...
For comparison, we calculated the total CPO score across a range of cutoff values, and compared these results to the true predictive density using LOOCV.
For our comparison using a larger test set (25 observations), we calculated the predictive log density for each observation.
Additional metrics, including the bias and standard error associated with each prediction were also used for comparisons.
...
Finally, we used our groundfish examples to explore the robustness of predictive performance via cross-validation to the choice of hyperparameters for the PC priors (Matérn $\kappa$ and $\sigma_{\omega}$).
Rather than do a prior sensitivity across all species and sub-models, we applied this sensitivity analysis to the Dover Sole presence-absence sub-model.
The PC prior on $\sigma_{\omega}$ is to specify the threshold and upper tail probability, $Pr(\sigma_{\omega} > \sigma_{0}) = \theta_{\sigma}$.
In addition to the original values ($\sigma_{0}$ = 5, $\theta_{\sigma}$ = 0.05), we examined alternative values of $\sigma_{0}$ = (1, 10) and $\theta_{\sigma}$ = c(0.01, 0.1, 0.4).
In contrast, the PC prior on $\kappa$ specifies the lower threshold and tail probability, $Pr(\kappa > \kappa_{0}) = \theta_{\kappa}$.
We varied $\kappa_{0}$ (5, 20, 40) and $\theta_{\kappa}$ (0.01, 0.1, 0.4).
For each combination of PC priors, we calculated the out of sample predictive density, using 10-fold cross-validation with 20 latitudinal bands defined by quantiles.

We validated parameters estimates from our estimation with `inlabru` by also estimating the same models using TMB [@kristensen_tmb_2016].
Specifically, we performed estimation using the R package 'sdmTMB' [@anderson_2022], which uses the INLA software to create meshes, but takes advantage of automatic differentiation in TMB for faster maximum likelihood parameter estimation.
