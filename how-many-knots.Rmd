...
While previous research has suggested that increasing the resolution of the mesh improves predictive accuracy [@godana_dynamic_2019; @wilson_evaluating_2021; @williamson_spatiotemporal_nodate; @thorson_guidance_2019], other authors have also noted that finer meshes don't always translate into better performance [@huang_evaluating_2017].

The objective of this paper is to demonstrate that higher resolution spatial approximations in INLA may not translate to better predictive accuracy.
Using a series of analyses on a real-world case study of groundfish data in the Northeast Pacific Ocean, we evaluate the out-of-sample predictive performance as a function of mesh resolution (number of vertices).
...
We adopt a similar approach here, using a binomial distribution with a logit link to model species occurrences, and a Gamma distribution with a log link to model positive catch rates.
...
where $\omega_{s}$ is the estimated spatial field, $B_0$ is an intercept, $u_s$ is the expected value for point in space $s$, and $g()$ is an inverse link function.
...
Throughout our analyses, we adjusted the mesh resolution by varying the cutoff distance (representing the distance where two points are treated as a single vertex) because the cutoff has been shown to have the greatest impact on measures of model performance from mesh construction [@righetto_choice_2020].
...
We used penalized complexity priors on the Matérn covariance parameters [@simpson_penalising_2015; @osgood-zimmerman_statistical_2021] with TODO (AND OTHER PRIORS).
...
The high-resolution meshes with more vertices than observations were included as an upper extreme, though many applications of INLA to similar survey data have used 500--600 vertices [e.g., @thorson_guidance_2019; @tolimieri_spatio-temporal_2020].
...
We a priori divided the survey domain into 10 spatial blocks by latitude quantiles with approximately the same number of points in each box.
Each model was run using inlabru [\@ @bachl_inlabru_2019], and used to predict the data for the held out test data.
...
Using the presence-absence data from Dover Sole, we examined the cross validation performance across a range of mesh resolutions (cutoff values ranging from 15--75, or 122--594 vertices), number of spatial blocks (19--218) and number of folds used for cross validation (10, 40).
...
Fig.
1 just demonstrates the data used in the study, and a couple mesh configurations (coarse and fine, cutoff of 80 and 20, translating to 110 and 394 vertices)
...
-   I think the spatial range and SD would be (most?) interesting here
